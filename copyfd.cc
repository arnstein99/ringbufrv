...
{
public:
    IOPackage(int rdfd, int wrfd, size_t bufsiz)
         : readfd(rdfd), writefd(wrfd), bufr(bufsiz) { }
...
copyfd_stats copyfd(int readfd, int writefd, size_t buffer_size)
{
    ...
    IOPackage pack(readfd, writefd, buffer_size);
    ...
    copyfd_stats stats[2])
{
    ...
    pfd[2].fd = writefd;
    pfd[3].fd = readfd;
    IOPackage forward(readfd, writefd, buffer_size);
    IOPackage backward(writefd, readfd, buffer_size);
...
bool IOPackage::cycle(pollfd pfd[2])
{
    pfd[0].events = 0;
    pfd[1].events = 0;
    pfd[0].revents = 0;
